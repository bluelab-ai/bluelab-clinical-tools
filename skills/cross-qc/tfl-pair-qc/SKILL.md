...
- File MUST start with exactly 3 consecutive META lines (no blank lines between):
  ...
  ```
- `##META_CONCLUSION:` = highest severity among all findings, or `PASS` if none
...
|-------|-----------|----------|
| **Critical** | Affects primary conclusions, endpoints, safety, or regulatory judgment | Primary endpoint unverifiable from listing; death count mismatch; SS grouping error |
| **Major** | Affects report quality, traceability, or reviewer understanding | Table has no corresponding listing; analysis set mismatch between title and denominator |
| **Minor** | Format, table numbers, footnotes, version, coding | Table number reference error; N format inconsistency; rounding ≤0.2% |
| **Suggestion** | Not an error, could be clearer | Add footnote for special subjects; suggest supplementing missing listing |
