...
_PDF_TITLE_SYSTEM_PROMPT = """你是临床试验文档解析专家。给定 PDF 文档各页的文本，识别哪些页面包含表格/清单标题，并提取标题文本。

返回 JSON 数组（仅 JSON，不要其他内容）：
[{"page": 3, "title": "表 11.1.1.1 各中心病例分布（FAS）", "is_continued": false}, ...]

规则：
1. 只提取表格/清单的标题行（以"表"/"Table"/"表格"/"清单"/"Listing"开头并带编号的行）
2. 不要提取章节标题（如"11.1 不良事件"）、页眉页脚、目录条目
3. 如果同一页有多个表格标题，每个都列出
4. 标题含"续表"/"Continued"时 is_continued 为 true
5. 无表格标题的页面不要列出
6. page 是输入的页面编号（从 1 开始）"""
...
def extract_title(page, doc_type):
    """正则提取标题：表格用 '表 X.X.X.X ...' 模式，清单用 '清单 N ...' 模式（LLM 不可用时的兜底）"""
...
def _batch_llm_extract_titles(page_texts: list[tuple[int, str]], doc_type: str,
                               api_key: str, api_base=LLM_API_BASE,
                               model=LLM_MODEL) -> dict[int, str] | None:
    """批量调用 LLM 从所有页面文本中提取表格标题映射。

    Args:
        page_texts: [(page_num, page_text), ...]  非目录页，有文本
        doc_type: "表格" | "清单"

    Returns: {page_num: title, ...}  |  None（LLM 不可用或失败）
    """
    if not page_texts or not api_key:
        return None
    ...
    for attempt in range(max_retries):
        ...
        for pn, text in page_texts:
            # 只取每页前 500 字符（标题通常在页首）
            truncated = text[:500] if len(text) > 500 else text
            page_blocks.append(f"[第{pn}页]\n{truncated}")
        ...
        try:
            resp = client.messages.create(
                ...
                system=_PDF_TITLE_SYSTEM_PROMPT,
                messages=[{"role": "user", "content": user_prompt}],
            )
        except Exception as e:
            print(f"  ⚠ LLM 批量标题提取失败 (attempt {attempt + 1}/{max_retries}): {e}")
        ...
        try:
            ...
            if not m:
                if attempt < max_retries - 1:
                    print(f"  ⚠ LLM 输出无 JSON 数组，{(attempt + 1) * 2}s 后重试 ({attempt + 1}/{max_retries})...")
                    time.sleep((attempt + 1) * 2)
                    continue
                return None
            ...
            for item in items:
                ...
                if pn > 0 and title:
                    result[pn] = title
            print(f"  ✅ LLM 提取了 {len(result)} 个标题")
            return result
...
def process_pdf(pdf_path, output_dir, doc_type,
    ...
    # ── 批量 LLM 标题提取（所有非目录页一次调用）──
    llm_titles: dict[int, str] = {}
    if api_key:
        page_texts = []
        for pn, page in enumerate(pdf.pages, 1):
            if not is_toc_page(page, doc_type):
                text = page.extract_text()
                if text:
                    page_texts.append((pn, text))
        result = _batch_llm_extract_titles(
            page_texts, doc_type, api_key,
            api_base or LLM_API_BASE, model or LLM_MODEL,
        )
        if result:
            llm_titles = result
    ...
    for pn, page in enumerate(pdf.pages, 1):
        ...
        # 优先 LLM 结果，回退正则
        title = llm_titles.get(pn) or extract_title(page, doc_type)
...
def main():
    ...
    if not files:
        print("错误: 无有效 PDF 文件"); sys.exit(1)
