...
- Apply few-shot format constraints and output-rule prompts from the bundled `assets/fewshot.md` AFTER semantic review correction. The file defines per-field reference examples and output-rule prompts (e.g. semantic boundary splitting rules); reformat the corrected trace values to match the example style and extraction rules before drafting. Users may override with a custom `fewshot.md` via `--fewshot`.
...
3. **Semantic review of high-risk fields** (new). The trace extractors use pure rules (table structure, regex, keyword matching) without semantic understanding. Fields such as `样本量`, `研究设计`, `主要有效性终点`, `其他终点`, and `统计分析人群` are prone to extraction errors — e.g., picking an intermediate sample size (184例) instead of the final total (205例). Run semantic review BEFORE drafting:
   ...
   # Step 3d: Few-shot format constraint.
   # Constrain the output style of fields to match reference examples and output-rule
   # prompts in assets/fewshot.md. This step runs AFTER semantic review so values are
   # already corrected before reformatting.
...
- `assets/fewshot.md`: per-field few-shot formatting examples and output-rule prompts (e.g. semantic boundary splitting rules for 试验概述 fields). Used as the default unless user provides a custom file via `--fewshot`.
