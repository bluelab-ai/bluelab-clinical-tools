...
def default_fewshot_path() -> Path:
    """Return the bundled assets/fewshot.md path relative to this script."""
    return Path(__file__).resolve().parent.parent / "assets" / "fewshot.md"
...
def main() -> None:
    ...
    parser.add_argument("--fewshot", type=Path, help="fewshot.md path (defaults to assets/fewshot.md)")
    parser.add_argument("--review", type=Path, help="review JSON path (for apply, defaults to --out)")
    parser.add_argument("--out", required=True, type=Path)
    args = parser.parse_args()

    if args.mode == "prepare":
        fewshot_path = args.fewshot or default_fewshot_path()
        if not fewshot_path.exists():
            print(f"Few-shot file not found: {fewshot_path} – skipping few-shot formatting.")
            out = {"metadata": {}, "fewshot_path": str(fewshot_path), "review_items": []}
            args.out.parent.mkdir(parents=True, exist_ok=True)
            args.out.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
            return
        cmd_prepare(args.trace, fewshot_path, args.out)
    else:
