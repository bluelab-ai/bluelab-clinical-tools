#!/usr/bin/env python3
"""
F_G1_QUALSUB_Y_N 填充脚本（定性指标亚组-单组-有缺失）
按亚组分组，每个亚组内为多个指标重复4行统计结构
"""
...
def fill_template(semantic, projects, subgroups):
    ...
    for proj in projects:
        ...
        # 构建行模板：例数(缺失)、各分类 n(%)、分类1的双侧95% CI(%)、分类2 n(%)
        rows = []
...
def main():
    ...
    semantic_path = os.path.join(SCRIPT_DIR, "F_G1_QUALSUB_Y_N.json")
