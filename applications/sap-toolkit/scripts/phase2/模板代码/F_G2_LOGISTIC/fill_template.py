#!/usr/bin/env python3
"""
F_G2_LOGISTIC 填充脚本（Logistic回归分析表）
定性指标，带置信区间和OR值
"""
...
def fill_template(semantic, projects):
    ...
    for proj in projects:
        ...
        # 添加OR行（只在最后添加一次）
        for row_idx, row in enumerate(sections[0]["rows"]):
            if row_idx >= 3 and not row.get("is_separator", False):
                label_vals = row.get("label_values", [])
                data_vals = row.get("data_values", [])
                applies_to = row.get("applies_to", [])
                metric = label_vals[1] if len(label_vals) > 1 else ""
...
def main():
    ...
    semantic_path = os.path.join(SCRIPT_DIR, "F_G2_LOGISTIC.json")
