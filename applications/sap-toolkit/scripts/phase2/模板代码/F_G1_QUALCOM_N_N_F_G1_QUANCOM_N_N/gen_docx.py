...
def main():
    parser = argparse.ArgumentParser(
        description="基于填充好的 JSON 生成三线表 Word 文档（混合定性+定量指标）"
    )
    ...
    qual_count = sum(1 for i in filled_data.get("indicators", []) if not i.get("unit"))
    quant_count = indicator_count - qual_count
    print(f"✓ 已生成: {output_path}")
    print(f"  包含 {indicator_count} 个指标 (定性: {qual_count}, 定量: {quant_count})（三线表格式）")
