#!/usr/bin/env python3
"""
F_G3_QUALSUB_Y_N 填充脚本（定性指标亚组-三组-有缺失）
按亚组分组，每个亚组内为多个指标重复7行统计结构
行结构：例数(缺失)、分类1 n(%)、分类1的双侧95% CI(%)、分类2 n(%)、统计方法、检验统计量、P值
"""
...
def fill_template(semantic, projects, subgroups):
    ...
    for proj in projects:
        ...
        # 构建行模板：例数(缺失)、分类1 n(%)、分类1的双侧95% CI(%)、分类2 n(%)、统计方法、检验统计量、P值
        rows = []
        for j, row in enumerate(row_template):
            ...
            if metric.startswith("分类") and categories:
                ...
                if cat_idx < len(categories):
                    # 处理 "分类1 n(%)" 和 "分类1的双侧95% CI(%)" 两种情况
                    if "CI" in metric:
                        metric = f"{categories[cat_idx]}的双侧95% CI(%)"
                    else:
...
def main():
    ...
    semantic_path = os.path.join(SCRIPT_DIR, "F_G3_QUALSUB_Y_N.json")
