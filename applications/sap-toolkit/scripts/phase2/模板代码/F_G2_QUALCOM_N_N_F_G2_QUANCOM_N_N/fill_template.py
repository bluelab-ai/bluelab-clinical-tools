#!/usr/bin/env python3
"""
F_G2_QUALCOM_N_N_F_G2_QUANCOM_N_N 填充脚本
混合定性+定量指标的两组比较表

判断逻辑：
- 有 categories 字段 → 定性指标，按 F_G2_QUALCOM_N_N 方式填充
- 有 unit 字段且无 categories → 定量指标，按 F_G2_QUANCOM_N_N 方式填充
"""
...
def fill_qualitative(qual_section, categories, display):
    """填充定性指标（按 F_G2_QUALCOM_N_N 方式）

    参数:
        qual_section: 定性指标的section模板
        categories: 分类列表，如 ["男", "女"]
        display: 指标显示名称
    ...
    for row in qual_section.get("rows", []):
...
def fill_quantitative(quant_section, display):
    """填充定量指标（按 F_G2_QUANCOM_N_N 方式）

    参数:
        quant_section: 定量指标的section模板
        display: 指标显示名称（含单位）
...
def main():
    ...
    semantic_path = os.path.join(SCRIPT_DIR, "F_G2_QUALCOM_N_N_F_G2_QUANCOM_N_N.json")
