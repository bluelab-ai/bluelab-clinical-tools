#!/usr/bin/env python3
"""
F_G2_QUALCOM_Y_N_F_G2_QUANCOM_Y_N 填充脚本
混合定性+定量指标的两组比较表（有缺失）

判断逻辑：
- 有 categories 字段 → 定性指标，按 F_G2_QUALCOM_Y_N 方式填充（每个分类 n(%) + 95%CI）
- 有 unit 字段且无 categories → 定量指标，按 F_G2_QUANCOM_Y_N 方式填充（含均值/中位数95%CI）
"""
...
def fill_qualitative(qual_section, categories, display):
    """填充定性指标（按 F_G2_QUALCOM_Y_N 方式，每个分类 n(%) + 95%CI）"""
    rows = []
...
def fill_quantitative(quant_section, display):
    """填充定量指标（按 F_G2_QUANCOM_Y_N 方式，含均值/中位数95%CI）"""
    rows = []
    first = True
...
def main():
    ...
    semantic_path = os.path.join(SCRIPT_DIR, "F_G2_QUALCOM_Y_N_F_G2_QUANCOM_Y_N.json")
