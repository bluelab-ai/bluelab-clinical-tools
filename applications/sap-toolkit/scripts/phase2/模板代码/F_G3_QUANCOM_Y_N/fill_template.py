#!/usr/bin/env python3
"""
F_G3_QUANCOM_Y_N 填充脚本（多组定量-有缺失）
...
def fill_template(semantic, projects):
    """
    将指标数据填入 F_G3_QUANCOM_Y_N 语义模板

    参数:
        semantic: F_G3_QUANCOM_Y_N.json 语义模板
    ...
    for proj in projects:
        ...
        for section in sections:
            row = section["rows"][0]
            metric = row.get("label_values", ["", ""])[1] if len(row.get("label_values", [])) > 1 else ""
...
def main():
    ...
    semantic_path = os.path.join(SCRIPT_DIR, "F_G3_QUANCOM_Y_N.json")
