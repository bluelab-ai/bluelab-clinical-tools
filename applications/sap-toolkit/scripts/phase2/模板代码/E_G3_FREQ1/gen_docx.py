...
def generate_docx(filled_data, output_path):
    ...
    # 合并第一列（项目）的两行表头
    merge_cells_vertical(table, 0, 1, 0)

    # 合并表头行1：组别1 (列2-3), 组别2 (列4-5), 合计 (列6-7)
    merge_cells(table, 0, 2, 3)  # 组别1
    merge_cells(table, 0, 4, 5)  # 组别2
    ...
    for i, row_data in enumerate(rows):
        ...
        if indent > 0 and metric:
            metric = "    " + metric  # 4个空格缩进
        set_cell_text(table.rows[row_idx].cells[0], metric)

        # 第2列：指标
        indicator = label_vals[1] if len(label_vals) > 1 else ""
        set_cell_text(table.rows[row_idx].cells[1], indicator)
