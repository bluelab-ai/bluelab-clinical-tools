...
def apply_three_line_borders(table):
    ...
    for i, row in enumerate(table.rows):
        for j, cell in enumerate(row.cells):
            # 合并的单元格（第一列和最后一列）需要上下框线
            if (j == 0 or j == 7) and i <= 1:
                set_cell_borders(
                    cell,
                    top={"sz": 4, "color": "auto"},
...
def generate_docx(filled_data, output_path):
    ...
    # 合并表头行1：试验组 (列1-2), 对照组 (列3-4), 合计 (列5-6)
    merge_cells(table, 0, 1, 2)  # 试验组
    merge_cells(table, 0, 3, 4)  # 对照组
    merge_cells(table, 0, 5, 6)  # 合计
    ...
    # 合并最后一列（P值）的两行表头
    cell_start = table.rows[0].cells[7]
    cell_end = table.rows[1].cells[7]
    cell_start.merge(cell_end)

    # 填写表头行1
    set_cell_text(table.rows[0].cells[0], "项目", WD_ALIGN_PARAGRAPH.CENTER)
    set_cell_text(table.rows[0].cells[1], "试验组\n(N=n1)", WD_ALIGN_PARAGRAPH.CENTER)
    set_cell_text(table.rows[0].cells[3], "对照组\n(N=n1)", WD_ALIGN_PARAGRAPH.CENTER)
    set_cell_text(table.rows[0].cells[5], "合计\n(N=n1)", WD_ALIGN_PARAGRAPH.CENTER)
    set_cell_text(table.rows[0].cells[7], "P值", WD_ALIGN_PARAGRAPH.CENTER)

    # 填写表头行2
    set_cell_text(table.rows[1].cells[0], "项目", WD_ALIGN_PARAGRAPH.CENTER)
    ...
    set_cell_text(table.rows[1].cells[7], "P值", WD_ALIGN_PARAGRAPH.CENTER)
