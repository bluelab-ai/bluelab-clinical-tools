...
def fill_template(semantic, projects):
    ...
    for proj in projects:
        ...
        if name:
            rows.append({
                "label_values": [name],
                "data_values": ["x", "n2(x.x %)", "x", "n2(x.x %)", "x", "n2(x.x %)", "x.xxx"],
                "applies_to": ["试验组 例次", "试验组 人数(%)", "对照组 例次", "对照组 人数(%)", "合计 例次", "合计 人数(%)", "P值"],
                "indent": 0
            })
...
def main():
    ...
    semantic_path = os.path.join(SCRIPT_DIR, "E_G2_FREQ1.json")
